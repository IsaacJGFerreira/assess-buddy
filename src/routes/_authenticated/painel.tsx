...
import { listAvaliacoes, listTurmas, STATUS_LABEL } from "@/lib/domain";
...
function Painel() {
  ...
  const turmaNome = (id: string | null) =>
    turmas.data?.find((t) => t.id === id)?.nome ?? "—";
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-end justify-between mb-8">
        ...
        <Button asChild><Link to="/avaliacoes/nova"><Plus className="h-4 w-4 mr-2" />Nova avaliação</Link></Button>
      </div>
      ...
      {avals.isLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-border p-12 text-center">
          ...
          <Button asChild className="mt-4"><Link to="/avaliacoes/nova">Criar avaliação</Link></Button>
        </div>
      ) : (
              ...
              {avals.data.map((a) => (
                <tr key={a.id} className="border-t border-border hover:bg-muted/30">
                  <td className="px-4 py-3">
                    <Link to="/avaliacoes/$id" params={{ id: a.id }} className="font-medium text-primary hover:underline">
                      {a.titulo}
                    </Link>
                  </td>
